import { AptosAccount, AptosClient, Types } from 'movement-sdk'
import EventEmitter from 'eventemitter3'
import { testnet } from '../../chain'
import { UserRejectedRequestError } from '../../errors'
import { Address } from '../../types'
import { Account, Movement, SignMessagePayload, SignMessageResponse } from '../types'
...
const movementAccountToAccount = (acc: AptosAccount): Account => {
  return {
...
const client = new AptosClient(testnet.nodeUrls.default)
...
export class MockProvider implements Movement {
  ...
  async signAndSubmitTransaction(transaction: Types.EntryFunctionPayload): ReturnType<AptosClient['submitTransaction']> {
    if (!this._account) throw new UserRejectedRequestError(new Error())
  ...
  async signMessage(message?: SignMessagePayload | undefined): Promise<SignMessageResponse> {
    ...
    const application = 'movementlabs.xyz'
  ...
  async connect(): Promise<Account> {
    ...
    if (this._account) {
      this.events.emit('accountChange', movementAccountToAccount(this._account))
    }
    return this.account()
  }
  ...
  async account(): Promise<Account> {
    ...
    return movementAccountToAccount(this._account)
  }
